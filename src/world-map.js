...
/*globals window, document, DocumentTouch, jvm */
...
* @param {Function} params.onRegionLabelShow
*  <code>(Event e, Object label, String code)</code>
*  Will be called right before the region label is going to be shown.
* @param {Function} params.onRegionOver
*  <code>(Event e, String code)</code>
*  Will be called on region mouse over event.
* @param {Function} params.onRegionOut
*  <code>(Event e, String code)</code>
*  Will be called on region mouse out event.
* @param {Function} params.onRegionClick
*  <code>(Event e, String code)</code>
*  Will be called on region click event.
* @param {Function} params.onRegionSelected
*  <code>(Event e, String code, Boolean isSelected, Array selectedRegions)</code>
*  Will be called when region is (de)selected. <code>isSelected</code> parameter of the callback indicates whether region is selected or not. <code>selectedRegions</code> contains codes of all currently selected regions.
* @param {Function} params.onMarkerLabelShow
*  <code>(Event e, Object label, String code)</code>
*  Will be called right before the marker label is going to be shown.
* @param {Function} params.onMarkerOver
*  <code>(Event e, String code)</code>
*  Will be called on marker mouse over event.
* @param {Function} params.onMarkerOut
*  <code>(Event e, String code)</code>
*  Will be called on marker mouse out event.
* @param {Function} params.onMarkerClick
*  <code>(Event e, String code)</code>
*  Will be called on marker click event.
* @param {Function} params.onMarkerSelected
*  <code>(Event e, String code, Boolean isSelected, Array selectedMarkers)</code>
*  Will be called when marker is (de)selected. <code>isSelected</code> parameter of the callback indicates whether marker is selected or not. <code>selectedMarkers</code> contains codes of all currently selected markers.
* @param {Function} params.onViewportChange
*  <code>(Event e, Number scale)</code> Triggered when the map's viewport is changed (map was panned or zoomed).
*/
...
jvm.WorldMap.prototype = {
    ...
    /**
     * Synchronize the size of the map with the size of the container.
     * Suitable in situations where the size of the container is changed programmatically or container is shown after it became visible.
     */
    setSize: function () {
    ...
    bindContainerTouchEvents: function () {
        var touchStartScale, touchStartDistance, map = this,
            touchX, touchY, centerTouchX, centerTouchY, lastTouchesLength, handleTouchEvent = function (e) {
            var touches = e.originalEvent.touches,
                offset, scale, transXOld, transYOld;
            ...
            if (touches.length == 1) { //           ==?
                ...
                if (lastTouchesLength == 2) { //    ==?
                    scale = Math.sqrt(
                    Math.pow(touches[0].pageX - touches[1].pageX, 2) + Math.pow(touches[0].pageY - touches[1].pageY, 2)) / touchStartDistance;
    ...
    setScale: function (scale, anchorX, anchorY, isCentered) {
        var zoomStep, viewportChangeEvent = jvm.$.Event('zoom.jvectormap');
    ...
    /**
     * Set the map's viewport to the specific point and set zoom of the map to the specific level.
     * Point and zoom level could be defined in two ways: using the code of some region to focus on or a central point and zoom level as numbers.
     * @param {Number|String|Array} scale|regionCode|regionCodes
     *  If the first parameter of this method is a string or array of strings and there are regions with the these codes, the viewport will be set to show all these regions.
     *  Otherwise if the first parameter is a number, the viewport will be set to show the map with provided scale.
    ...
    setFocus: function (scale, centerX, centerY) {
        ...
        if (jvm.$.isArray(scale) || this.regions[scale]) {
            ...
            this.setScale(
            Math.min(this.width / bbox.width, this.height / bbox.height), - (bbox.x + bbox.width / 2), - (bbox.y + bbox.height / 2), true);
        } else {
     ...
     * @param {String|Array|Object} keys If <code>String</code> or <code>Array</code> the region(s) with the corresponding code(s) will be selected.
     *  If <code>Object</code> was provided its keys are  codes of regions, state of which should be changed. Selected state will be set if value is true, removed otherwise.
     */
     ...
     * @param keys {String|Array|Object}
     *  If <code>String</code> or <code>Array</code> the marker(s) with the corresponding code(s) will be selected.
     *  If <code>Object</code> was provided its keys are  codes of markers, state of which should be changed.
     *  Selected state will be set if value is true, removed otherwise.
     */
     ...
     * @param markers    {Object|Array}  Markers to add to the map.
     *  In case of array is provided, codes of markers will be set as string representations of array indexes.
     * @param seriesData {Array}         Values to add to the data series.
     */
     ...
     * @param markers {Array}  Array of marker codes to be removed.
     */
     ...
     * @param lat {Number}  Latitide of point in degrees.
     * @param lng {Number}  Longitude of point in degrees.
     */
    latLngToPoint: function (lat, lng) { // unused?
        var point, proj = jvm.WorldMap.maps[this.params.map].projection,
            centralMeridian = proj.centralMeridian,
            width = this.width - this.baseTransX * 2 * this.baseScale, // unused?
     ...
     * @param x {Number}  X-axis of point on map in pixels.
     * @param y {Number}  Y-axis of point on map in pixels.
     */
    ...
    createSeries: function () {
        ...
        for (key in this.params.series) {
            for (i = 0; i < this.params.series[key].length; i++) {
                ...
                this.params.series[key][i], this[key]);
            }
        }
    },
